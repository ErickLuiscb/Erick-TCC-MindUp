...
export default function CriarVideo() {
  ...
  async function salvar(e) {
    ...
    if (!arquivo) {
      setErro(
        "❌ Por favor, selecione um arquivo de vídeo físico para fazer o upload.",
      );
      return;
    }
      ...
      setErro(
        "❌ " +
          (error.response?.data?.message ||
            "Erro ao tentar salvar o vídeo no servidor."),
      );
    } finally {
      setSalvando(false);
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 border border-purple-100">
        <header className="mb-6 border-b border-gray-100 pb-4">
          <h1 className="text-2xl font-black text-purple-950 tracking-wide">
            Publicar Novo Conteúdo em Vídeo 📹
          </h1>
          <p className="text-gray-500 text-xs mt-1">
            Preencha os campos e anexe o arquivo de vídeo. O upload aceita
            formatos até 50MB.
          </p>
        </header>
        ...
        <form onSubmit={salvar} className="space-y-5">
          <div>
            ...
            <input
              ...
              placeholder="Digite um título claro para a publicação..."
              required
              maxLength={150}
              disabled={salvando}
            />
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            >
              <Save size={14} />
              <span>
                {salvando ? "Enviando Mídia..." : "Publicar Conteúdo"}
              </span>
            </button>
          </div>
        </form>
