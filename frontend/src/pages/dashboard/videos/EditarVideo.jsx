...
export default function EditarVideo() {
  ...
  useEffect(() => {
    async function carregarVideo() {
      try {
        setCarregando(true);
        const resp = await api.get(`/videos/${id}`);
        ...
        if (video.categorias && Array.isArray(video.categorias)) {
          const idsAntigos = video.categorias.map((c) => c.id);
          setCategoriasSelecionadas(idsAntigos);
        }
      } catch (error) {
        console.error(error);
        alert("Erro ao carregar os dados originais do vídeo.");
  ...
  const obtenerCategoriasAgrupadas = () => {
    const grupos = {};
  ...
  const categoriesAgrupadas = obtenerCategoriasAgrupadas();
      ...
      setErro(
        "❌ " +
          (error.response?.data?.message ||
            "Erro ao tentar salvar as modificações do vídeo."),
      );
      setSalvando(false);
    }
  }
    ...
    return (
      <div className="flex justify-center pt-20 animate-pulse">
        <p className="text-xl font-bold text-white tracking-wide">
          Recuperando registros do vídeo...
        </p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 border border-purple-100">
        <header className="mb-6 border-b border-gray-100 pb-4">
          <h1 className="text-2xl font-black text-purple-950 tracking-wide">
            Modificar Vídeo Publicado ✏️
          </h1>
          <p className="text-gray-500 text-xs mt-1">
            Altere as informações desejadas. Caso selecione um novo arquivo
            multimídia, o antigo será substituído do servidor automaticamente.
          </p>
        </header>
        ...
        <form onSubmit={salvar} className="space-y-5">
          ...
          <div>
            <label className="block mb-1 text-xs font-bold text-purple-950 uppercase tracking-wider">
              Trocar Mídia de Vídeo (Opcional):
            </label>
            <input
              type="file"
              accept="video/mp4,video/quicktime,video/x-msvideo"
              onChange={(e) => setArquivo(e.target.files[0])}
              className="w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:bg-purple-100 file:text-purple-700 hover:file:bg-purple-200 cursor-pointer border border-gray-200 p-2 rounded-xl bg-white"
              disabled={salvando}
            />
            <button
              type="button"
            ...
            <button
              type="submit"
              className="mt-6 w-full flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={salvando}
            >
